...
func (c *ResultController) Post() {
	...
	//生成uuid
	v.Uuid = strings.ReplaceAll(uuid.NewV4().String(), "-", "")
	//生成上传时间
	v.UploadTime = time.Now().Format("2006-01-02 15:04:05")
	//生成hash摘要
	bytes, _ := json.Marshal(v)
	m := sha256.New()
	m.Write(bytes)
	v.HashAbstract = hex.EncodeToString(m.Sum(nil))
